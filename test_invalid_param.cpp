...

{
    ...
    fs_session("wronguser", "passwordtoooooolong1", &session, seq++);
    ...
    fs_create("user1", "password1", session, seq++, "/failatcreate", 'd');
    ...
    // READBLOCK
    // wrong pwd, user, session num
    fs_readblock("user1", "wrongpwd1", session, seq++, "/dir/file", 0, readdata);
    fs_readblock("wronguser", "password1", session, seq++, "/dir/file", 0, readdata);
    fs_readblock("user3", "password3", session, seq++, "/dir/file", 0, readdata);

    // wrong dir / block
    fs_readblock("user1", "password1", session, seq++, "dir/file", 0, readdata);
    fs_readblock("user1", "password1", session, seq++, "/dir/file", 0, readdata);
    fs_readblock("user1", "password1", session, seq++, "/non/exist", 0, readdata);
    fs_readblock("user1", "password1", session, seq++, "/dir/file", -1, readdata);
    fs_readblock("user1", "password1", session, seq++, "/dir/file", 255, readdata);

    // wrong seq
    fs_readblock("user1", "password1", session, seq - 1, "/dir/file", 0, readdata);
